...
class ZLibraryAutoUploader:
    ...
    def convert_to_txt(self, file_path: Path, file_format: str = None) -> Path:
        """转换文件为 TXT 或直接使用 PDF"""
        print("")
        print("="*70)
        print("📝 处理文件")
        ...
        txt_file = self.temp_dir / f"{file_path.stem}.txt"

        # 如果是 EPUB，转换为 TXT
        if file_ext == '.epub':
            print("📖 检测到 EPUB 格式，转换为 TXT...")
            cmd = f"python3 /tmp/convert_epub.py '{file_path}' '{txt_file}'"
            import subprocess
    ...
    def upload_to_notebooklm(self, file_path: Path, title: str = None) -> dict:
        """上传到 NotebookLM"""
        print("")
        print("="*70)
